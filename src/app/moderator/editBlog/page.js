...
// // ========== MAIN COMPONENT ==========
// export default function ModeratorEditBlog() {
...
export default function ModeratorEditProduct() {
  ...
  // Combine existing and new images for unified display
  useEffect(() => {
    const combined = [];
    ...
    existingImages.forEach((img) => {
      combined.push({
        ...
        originalData: img,
        uploading: false,
        tempId: `existing-${img.publicId}`
      });
    });
    
    // Add uploaded new images
    newImages.filter(img => img.url && !img.uploading).forEach((img) => {
      combined.push({
        id: img.id,
        url: img.url,
        preview: img.preview,
  ...
  // Move image in unified array - handles ALL images together
  const moveImage = (fromIndex, toIndex) => {
    ...
    // Separate back into existing and new images after reorder
  ...
  const handleDragLeave = () => setDragOverImageIndex(null);
  ...
  const handleSubmit = async (e) => {
    ...
    const uploading = allImages.some(img => img.uploading === true);
    if (uploading) {
    ...
    try {
      ...
      // IMPORTANT: Use allImages for the exact order (preserves drag-drop sequence)
      const finalImageUrls = allImages
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-1 bg-[#F5E6D3] text-[#6B4F3A] text-xs font-medium rounded-full">Moderator</span>
                  </div>
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-xl shadow-sm border border-[#E8D5C0]">
                  <div className="p-5 border-b border-[#E8D5C0]">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Total: {allImages.filter(img => !img.uploading && img.url).length} of 6 images
                    </p>
                  </div>
                  <div className="p-5">
                            ...
                            return (
                              <div key={slotId} className="relative">
                                <input
                                  ...
                                  ref={el => { if (el) fileInputRefs.current[slotId] = el; }}
                                />
                                <button
                                  type="button"
                                  onClick={() => fileInputRefs.current[slotId]?.click()}
                                  className="w-full border-2 border-dashed border-gray-300 rounded-lg p-3 text-center transition-colors cursor-pointer h-24 flex flex-col items-center justify-center hover:border-[#4A7C59] hover:bg-[#F5E6D3]"
                                >
                    ...
                    <div className="mt-4 text-xs text-gray-500 text-center">
                      {allImages.filter(img => !img.uploading && img.url).length} of 6 images total
                    </div>
                  </div>
                </div>
              </div>
