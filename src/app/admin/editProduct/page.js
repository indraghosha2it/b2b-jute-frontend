...
export default function EditProduct() {
  ...
  // Combine existing and new images for unified display - INCLUDES UPLOADING IMAGES
  useEffect(() => {
    const existing = existingImages.map((img, idx) => ({
      id: img.publicId,
      url: img.url,
      type: 'existing',
    ...
    // Include both uploaded new images and uploading images
    const uploadedNew = newImages.filter(img => img.url).map((img, idx) => ({
      id: img.id,
      url: img.url,
      preview: img.preview,
      type: 'new',
    ...
    const uploadingNew = newImages.filter(img => img.uploading).map((img, idx) => ({
      id: img.id,
      preview: img.preview,
      type: 'new',
      isPrimary: false,
    ...
    setAllImages([...existing, ...uploadedNew, ...uploadingNew]);
  ...
  const moveImage = (fromIndex, toIndex) => {
    ...
    const reorderedExisting = updatedImages
      .filter(img => img.type === 'existing' && !img.uploading)
      .map(img => img.originalData);
    
    const reorderedNew = updatedImages
      .filter(img => img.type === 'new' && !img.uploading && img.url)
      .map(img => img.originalData);
  ...
  const handleNewImageChange = async (e, slotId) => {
    ...
    try {
      const { url, publicId } = await uploadToCloudinary(file);
      setNewImages(prev => prev.map(img => 
        img.id === imageId ? { ...img, url, publicId, uploading: false } : img
      ));
      ...
      <div className="min-h-screen bg-[#FAF7F2]">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-1 bg-[#F5E6D3] text-[#6B4F3A] text-xs font-medium rounded-full">Admin</span>
                  </div>
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit}>
            ...
            {/* Submit Button */}
            <div className="mt-6 flex justify-end gap-3">
              <NextLink href="/admin/all-products" className="px-6 py-3 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition-colors text-sm">Cancel</NextLink>
