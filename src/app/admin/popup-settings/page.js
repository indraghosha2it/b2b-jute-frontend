...
// import { Save, Loader2, Bell, Package, Settings, AlertCircle } from 'lucide-react';
...
//       const response = await fetch('https://b2b-jute-backend.vercel.app/api/popup-settings', {
...
//                     : 'border-gray-200 hover:border-[#6B4F3A]/50'
//                 }`}
//               >
//                 <Package className="w-8 h-8 mx-auto mb-2 text-[#6B4F3A]" />
//                 <p className="font-medium text-sm">Promotional Only</p>
...
//                 }`}
//               >
//                 <Bell className="w-8 h-8 mx-auto mb-2 text-[#6B4F3A]" />
//                 <p className="font-medium text-sm">Newsletter Only</p>
...
//           {/* Newsletter Settings */}
//           {(settings.activePopup === 'newsletter' || settings.activePopup === 'both') && (
//             <div className="p-6 border-b border-gray-200">
...
import { useState, useEffect } from 'react';
import { toast } from 'sonner';
import { Save, Loader2, Bell, Package, Settings, AlertCircle, Eye, CheckCircle, Calendar, User } from 'lucide-react';
...
export default function PopupSettingsPage() {
  ...
  const [dbData, setDbData] = useState(null); // Store actual database data
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log('📊 Newsletter Active:', data.data.newsletter?.isActive);
        console.log('📊 Promotional Active:', data.data.promotional?.isActive);
        
        setSettings(data.data);
        setDbData(data.data); // Store the actual database data
      } else {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Popup settings saved successfully!');
        // Refresh settings after save to show updated data
        fetchSettings();
      } else {
        toast.error(data.error || 'Failed to save settings');
      }
  ...
  // Get active popup display info from database
          ...
          <div className="mb-6 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-5">
              ...
              {/* <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-xs font-semibold text-blue-800 mb-2">📊 Popup Values:</p>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-gray-600">activePopup:</span>
                    <code className="ml-1 text-blue-800 font-mono">{dbData.activePopup}</code>
                  </div>
                  <div>
                    <span className="text-gray-600">priority:</span>
                    <code className="ml-1 text-blue-800 font-mono">{dbData.priority}</code>
                  </div>
                  <div>
                    <span className="text-gray-600">newsletter.isActive:</span>
                    <code className="ml-1 text-blue-800 font-mono">{dbData.newsletter?.isActive ? 'true' : 'false'}</code>
                  </div>
                  <div>
                    <span className="text-gray-600">promotional.isActive:</span>
                    <code className="ml-1 text-blue-800 font-mono">{dbData.promotional?.isActive ? 'true' : 'false'}</code>
                  </div>
                </div>
              </div> */}
